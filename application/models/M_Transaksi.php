...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_makanan', $this->input->post('kode_makanan')[$i])->get('makanan')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_makanan', $this->input->post('kode_makanan')[$i])->update('makanan', $u_stok);
			}
			...
			$s_dttransaski[] = array('kode_transaksi' => $ts->kode_transaksi,
									 'kode_makanan'=>$this->input->post('kode_makanan')[$i],
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('makanan', 'makanan.kode_makanan = detail_transaksi.kode_makanan')
