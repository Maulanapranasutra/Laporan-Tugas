	...
	<div class="col-md-7">
		...
		<table class="table table-hover table-stripped">
			...
			<thead>
				...
				<td>no</td><td>makanan</td><td>tahun</td><td>kategori</td><td>harga</td><td>cover</td><td>stok</td><td></td><td></td>
			...
			<tbody>
				...
				<tr>
					...
					<td><?=$no?></td></td><td><?=$bk->makanan?></td><td><?=$bk->tahun?></td><td><?=$bk->nama_kategori?></td><td><?=$bk->harga?></td><td><img src="<?=base_url(''.$bk->cover)?>" style="width:40px;"></td><td><?=$bk->stok?></td>

					<td> <a href="<?=base_url('index.php/Cart/tambah_cart/'.$bk->kode_makanan)?>" class="btn btn-success">Beli</a></td>
...
<table class="table table-stripped table-hover">
	...
	<tr><td colspan="2">Nama Pembeli</td><td colspan="4"><input type="text" name="pembeli" value="<?=$this->session->flashdata('pembeli');?>"></td></tr>
				...
				<th>No</th><th>makanan</th><th>Jumlah</th><th>Harga</th><th>Subtotal</th><th></th>
			...
			<?php $no = 0; foreach($this->cart->contents() as $ct): $no++;?>
				...
				<input type="hidden" name="kode_makanan[]" value="<?=$ct['id']?>">
